...
lines = open("PropK1K2.txt").readlines()
for l in lines:
  if "IC50" in l:
    s = l.split()
    Prop50.append(float(s[5]))

lines = open("OurK1K2.txt").readlines()
for l in lines:
  if "IC50" in l:
    s = l.split()
    Our50.append(float(s[5]))

lines = open("OptK1K2.txt").readlines()
for l in lines:
  if "IC50" in l:
    s = l.split()
...
plotter.PlotN([R, R], [i1, i2], 
    X='Ratio of Fanout $k_1 / k_2$', Y='% Improvement in \n Response Quality', \
    labels=['Cedar', 'Ideal'], legendSize=18, legendLoc='lower right', \
    lWidth=4, mSize=6, \
    yAxis=[0, 100.1], #xAxis=[139.99, 170.01],\
    outputFile="PercImpK1K2")
