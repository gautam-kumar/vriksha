...
void TcpHeader::SetReserved (uint8_t reserved)
{
	m_reserved = reserved;
}
...
uint8_t  TcpHeader::GetReserved () const
{
  return m_reserved;
}
...
void TcpHeader::Serialize (Buffer::Iterator start)  const
{
  ...
  i.WriteHtonU16 (m_length << 12 | m_reserved << 8 | m_flags); //reserved bits are all zero <Gautam used for ECN>
