...
NS_LOG_COMPONENT_DEFINE ("D2Tcp");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (D2Tcp);

TypeId
D2Tcp::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::D2Tcp")
    .SetParent<TcpSocketBase> ()
    .AddConstructor<D2Tcp> ()
    .AddAttribute ("ReTxThreshold", "Threshold for fast retransmit",
                    UintegerValue (3),
                    MakeUintegerAccessor (&D2Tcp::m_retxThresh),
                    MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("LimitedTransmit", "Enable limited transmit",
		    BooleanValue (false),
		    MakeBooleanAccessor (&D2Tcp::m_limitedTx),
		    MakeBooleanChecker ())
    .AddTraceSource ("CongestionWindow",
                     "The TCP connection's congestion window",
                     MakeTraceSourceAccessor (&D2Tcp::m_cWnd))
  ;
  return tid;
}
...
/** We initialize m_cWnd from this function, after attributes initialized */
int
D2Tcp::Connect (const Address & address)
{
...
D2Tcp::Fork (void)
{
  return CopyObject<D2Tcp> (this);
}


void
D2Tcp::ConnectionSucceeded ()
...
void
D2Tcp::DoForwardUp (Ptr<Packet> packet, Ipv4Header header, uint16_t port,
...
void
D2Tcp::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  NS_LOG_DEBUG("Setting Absolute Deadline to " << m_absDeadline);
  ...
  if (tcpflags == 0)
    { // Bare data, accept it and move to ESTABLISHED state. This is not a normal behaviour. Remove this?
      ...
      Simulator::ScheduleNow (&D2Tcp::ConnectionSucceeded, this);
    }
...
void
D2Tcp::ProcessSynRcvd (Ptr<Packet> packet, const TcpHeader& tcpHeader,
...
void
D2Tcp::ProcessWait (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
D2Tcp::ProcessEstablished (Ptr<Packet> packet, const TcpHeader& tcpHeader, bool isEcnMarked)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
D2Tcp::ProcessLastAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
D2Tcp::PeerClose (Ptr<Packet> p, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
D2Tcp::SendEmptyPacket (uint8_t flags)
{
  ...
  if (m_retxEvent.IsExpired () && (hasSyn || hasFin) && !isAck )
    { // Retransmit SYN / SYN+ACK / FIN / FIN+ACK to guard against lost
      ...
      m_retxEvent = Simulator::Schedule (m_rto, &D2Tcp::SendEmptyPacket, this, flags);
    }
}
...
void
D2Tcp::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader, bool isEcnMarked)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
D2Tcp::ReceivedData (Ptr<Packet> p, const TcpHeader& tcpHeader, bool isEcnMarked)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
D2Tcp::ModifyCWnd ()
{
  ...
  uint32_t bytes = m_txBuffer.Size();
  Time current = Simulator::Now();
  double tc1, tc2;
  int64_t deadlineRemaining = ((m_absDeadline - current).GetInteger()) / (rtt.GetInteger()) - 1; // Number of RTTs to finish
  int64_t cWndInMss        = m_cWnd / m_segmentSize;
  int64_t bytesInMss       = floor (((double) bytes / (double) m_segmentSize) + 0.50);
  double  gamma1, gamma2;
  
  tc1 = std::sqrt(std::pow(((double)cWndInMss + 1.0)/2.0, 2.0) + 2.0 * (double)bytesInMss) - ((double)cWndInMss + 1.0)/2.0;
  if (deadlineRemaining > 0 && tc1 > 0.0)
    gamma1 = tc1 / (double) deadlineRemaining;
  else
    gamma1 = 0.0;
  
  tc2 = (double) bytesInMss / (3.0 * (double) cWndInMss / 4.0 + 0.5);
  if (deadlineRemaining > 0 && tc2 > 0.0)
    gamma2 = tc2 / (double) deadlineRemaining;
  else
    gamma2 = 0.0;
  
  double gammaLast = std::max(gamma1, gamma2);
  
  m_gamma = ((1 - GAMMA_WEIGHT) * m_gamma) + (GAMMA_WEIGHT * gammaLast);
  ...
  double p;
  if (m_gamma != 0.0)
    p = std::pow(m_dcTcpAlpha, m_gamma);
  else
    p = 0.0;

  m_cWnd = floor(((double) m_cWnd * (1.0 - p/2.0)) + 0.5);
}


/** New ACK (up to seqnum seq) received. Increase cwnd and call TcpSocketBase::NewAck() */
void
D2Tcp::NewAck(const SequenceNumber32& seq, bool hasEce, uint32_t packetSize)
{
  ...
  NS_LOG_LOGIC ("TcpNewReno receieved ACK for seq " << seq <<
	...
	if (packetSize <= 0) { // There is no data in this ACK
		...
		if (hasEce) {
			NS_LOG_INFO("Decreasing Congestion Window with alpha " << m_dcTcpAlpha);
			ModifyCWnd();
		}
	}
...
void
D2Tcp::DupAck (const TcpHeader& t, uint32_t count)
{
  NS_LOG_FUNCTION (this << count);
  if (count == m_retxThresh && !m_inFastRec)
    { // triple duplicate ack triggers fast retransmit (RFC2582 sec.3 bullet #1)
      m_ssThresh = std::max (2 * m_segmentSize, BytesInFlight () / 2);
			ModifyCWnd(); // TODO: Fix this properly
...
void
D2Tcp::SetSegSize (uint32_t size)
{
  NS_ABORT_MSG_UNLESS (m_state == CLOSED, "D2Tcp::SetSegSize() cannot change segment size after connection started.");
  m_segmentSize = size;
}

void
D2Tcp::SetSSThresh (uint32_t threshold)
{
  m_ssThresh = threshold;
}

uint32_t
D2Tcp::GetSSThresh (void) const
{
  return m_ssThresh;
}

void
D2Tcp::SetInitialCwnd (uint32_t cwnd)
{
...
uint32_t
D2Tcp::GetInitialCwnd (void) const
{
  return m_initialCWnd;
}

void 
D2Tcp::InitializeCwnd (void)
{
  /*
