...
TcpNewReno::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpNewReno")
    ...
    .AddTraceSource ("CongestionWindow",
		...
		.AddTraceSource ("SSThresh",
				"The TCP connection's slow start threshold",
...
TcpNewReno::NewAck (const SequenceNumber32& seq)
{
  ...
  if (m_inFastRec && seq < m_recover)
    { // Partial ACK, partial window deflation (RFC2582 sec.3 bullet #5 paragraph 3)
      if ((uint32_t) (seq - m_txBuffer.HeadSequence()) <= m_cWnd.Get()) { // <Fix by Gautam, was becoming negative>
...
TcpNewReno::DupAck (const TcpHeader& t, uint32_t count)
{
  ...
  if (count == m_retxThresh && !m_inFastRec)
    ...
    { // Increase cwnd for every additional dupack (RFC2582, sec.3 bullet #3)
      m_cWnd += m_segmentSize;
      NS_LOG_UNCOND (Simulator::Now().GetSeconds() << " Dupack in fast recovery mode. Increase cwnd to " << m_cWnd << " Rto: " << m_rto);
      SendPendingData (m_connected);
    }
  else if (!m_inFastRec && m_limitedTx && m_txBuffer.SizeFromSequence (m_nextTxSequence) > 0)
    { // RFC3042 Limited transmit: Send a new packet for each duplicated ACK before fast retransmit
      NS_LOG_UNCOND ("Limited transmit");
...
TcpNewReno::Retransmit (void)
{
  ...
  NS_LOG_UNCOND ("RTO. Reset cwnd to " << m_cWnd <<
