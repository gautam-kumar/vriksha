                           ...
                           Ipv4Address saddr, Ipv4Address daddr, Ptr<NetDevice> oif)
{
  ...
  packet->AddHeader(outgoingHeader);
	/*
  if ((outgoing.GetFlags() & TcpHeader::ECE) && (outgoing.GetFlags() & TcpHeader::ACK)) {
		TcpHeader temp;
		packet->RemoveHeader(temp);
		std::cout<<"\n\nIn TcpL4Protocol::PacketHeader " << ((int) temp.GetFlags())<< "\n";
		std::cout<<"\n\nIn TcpL4Protocol::OutgoingHeader " << ((int) outgoingHeader.GetFlags())<< "\n";
