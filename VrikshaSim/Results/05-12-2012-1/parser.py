...
def ParseNumCompleted(lines):
  Tcp = []; EdfNp = []; EdfP = []; Vriksha = []; SjfP = [];
  i = 0;
  for l in lines:
    regex = '; |, |\ |\*| \s+'
    s = re.split(regex, l)    
    if "TCP" in l:
      Tcp.append(float(s[4]));
    if "EDF-NP" in l:
      EdfNp.append(float(s[4]));
    if "EDF-P" in l:
      EdfP.append(float(s[4]));
    if "Vriksha" in l:
      Vriksha.append(float(s[4]));
    if "SJF-P" in l:
      SjfP.append(float(s[4]));
  return [Tcp, EdfNp, EdfP, Vriksha, SjfP]
